#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Pre-commit script to ensure that version numbers in `setup.json` and `aiida_abinit/__init__.py` match."""
import os
import json
import sys
...
def validate_version():
    """Check that version numbers in `setup.json` and `aiida_abinit/__init__.py` match."""
    sys.path.insert(0, FILEPATH_ROOT)
    import aiida_abinit  # pylint: disable=wrong-import-position
    version = aiida_abinit.__version__
    ...
    if version != setup_content['version']:
        ...
        click.echo(f"Version number in 'aiida_abinit/__init__.py': {version}")
